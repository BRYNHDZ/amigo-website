...
const Plan = () => {
  ...
  const selectOption = (category: SelectionKey, value: string, points: number = 0) => {
    ...
    if (category in scorePoints) {
      setScorePoints(prev => ({ ...prev, [category]: points }));
    }
  };
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <OptionCard
                      ...
                      title="Other nearby area"
                      description="I'm in a nearby surrounding city."
                    />
                  </div>
                </motion.div>
              )}
                ...
                >
                  ...
                  <div className="space-y-4">
                    <OptionCard
                      ...
                      onClick={() => selectOption("clippings", "The Nutrient Standard", 5)}
                      icon={Leaf}
                      title="The Nutrient Standard (Recommended)"
                      description="We mulch (shred) the clippings back into the lawn to act as a natural fertilizer for your soil."
                    />
                    <OptionCard
                      selected={selections.clippings === "The Pristine Standard"}
                      onClick={() => selectOption("clippings", "The Pristine Standard", 2)}
                      icon={Package}
                      title="The Pristine Standard"
                      description="We bag and haul away all clippings for a 100% clear surface every week."
                    />
                  </div>
                </motion.div>
              )}
                ...
                >
                  ...
                  <div className="space-y-4">
                    <OptionCard
                      ...
                      onClick={() => selectOption("lawnHealth", "Homeowner Managed", 2)}
                      icon={User}
                      iconColor="text-structure"
                      title="I'll handle this"
                      description="I will manage my own aeration, seeding, and treatment schedule."
                    />
                    <OptionCard
                      selected={selections.lawnHealth === "The Essential Refresh"}
                      onClick={() => selectOption("lawnHealth", "The Essential Refresh", 15)}
                      icon={Sprout}
                      title="The Essential Refresh"
                      description="Amigos handles professional Core Aeration and Overseeding in the Spring. Recommended to maintain density."
                    />
                    <OptionCard
                      selected={selections.lawnHealth === "The Elite Standard"}
                      onClick={() => selectOption("lawnHealth", "The Elite Standard", 20)}
                      icon={Award}
                      title="The Elite Standard"
                      description="Two professional aeration/seeding visits per year (Spring & Fall) for maximum density."
                    />
                  </div>
                ...
                >
                  ...
                  <div className="space-y-4">
                    <OptionCard
                      ...
                      onClick={() => selectOption("beds", "Homeowner Managed", 2)}
                      icon={User}
                      iconColor="text-structure"
                      title="I'll handle this"
                      description="I will handle my own weeding throughout the season."
                    />
                    <OptionCard
                      selected={selections.beds === "The Clean Look Plan"}
                      onClick={() => selectOption("beds", "The Clean Look Plan", 10)}
                      icon={Zap}
                      title='The "Clean Look" Plan'
                      description="A visual-first approach. We use string trimmers weekly to keep weeds at bay and ensure a clean transition."
                    />
                    <OptionCard
                      selected={selections.beds === "The Estate Detail Plan"}
                      onClick={() => selectOption("beds", "The Estate Detail Plan", 20)}
                      icon={Sparkles}
                      title='The "Estate Detail" Plan'
                      description="Our most thorough care. We hand-pull weeds from the root every week for a pristine bed surface."
                    />
                  </div>
                </motion.div>
              )}
              ...
              {currentStep === 7 && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button
                      type="button"
                      onClick={() => selectOption("refresh", "Perform Full Refresh", 10)}
                      className={`cursor-pointer transition-all duration-200 border-2 rounded-3xl bg-card p-6 flex flex-col items-center justify-center gap-2 text-center ${
                        selections.refresh === "Perform Full Refresh" 
                          ? "border-brand bg-[#F9FBE7] shadow-lg" 
                          : "border-border hover:border-highlight hover:-translate-y-0.5"
                      }`}
                    >
                      <CheckCircle className="w-10 h-10 text-brand mb-2" />
                      <div className="font-extrabold text-xl text-ink">Yes, handle it!</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => selectOption("refresh", "Homeowner Managed", 2)}
                      className={`cursor-pointer transition-all duration-200 border-2 rounded-3xl bg-card p-6 flex flex-col items-center justify-center gap-2 text-center ${
                        selections.refresh === "Homeowner Managed" 
                          ? "border-brand bg-[#F9FBE7] shadow-lg" 
                          : "border-border hover:border-highlight hover:-translate-y-0.5"
                      }`}
                    >
                ...
                >
                  ...
                  <div className="space-y-4">
                    <OptionCard
                      ...
                      onClick={() => selectOption("bushes", "Homeowner Managed", 2)}
                      icon={User}
                      iconColor="text-structure"
                      title="I'll handle this"
                      description="I will prune and shape my own bushes."
                    />
                    <OptionCard
                      selected={selections.bushes === "Annual Shaping"}
                      onClick={() => selectOption("bushes", "Annual Shaping", 10)}
                      icon={Scissors}
                      title="Annual Shaping (1 visit/yr)"
                      description="One professional visit per season to keep things neat."
                    />
                    <OptionCard
                      selected={selections.bushes === "Premium Managed Shaping"}
                      onClick={() => selectOption("bushes", "Premium Managed Shaping", 20)}
                      icon={Sparkles}
                      title="Premium Managed Support"
                      description="We monitor your bushes and shape them whenever they look shaggy."
                    />
                  </div>
              ...
              {currentStep === 9 && (
                ...
                >
                  ...
                  <div className="space-y-3">
                    <OptionCard
                      selected={selections.leaves === "Homeowner Managed"}
                      onClick={() => selectOption("leaves", "Homeowner Managed", 2)}
                      icon={User}
                      iconColor="text-structure"
                      title="I'll handle this"
                      description="I'll rake and bag my own leaves this fall."
                    />
                    <OptionCard
                      selected={selections.leaves === "Eco-Management"}
                      onClick={() => selectOption("leaves", "Eco-Management", 10)}
                      icon={RefreshCw}
                      title="Eco-Management (Tier 1)"
                      description="We mulch (shred) leaves weekly to feed natural nutrients into your soil."
                    />
                    <OptionCard
                      selected={selections.leaves === "Standard (Eco-Plus)"}
                      onClick={() => selectOption("leaves", "Standard (Eco-Plus)", 15)}
                      icon={CheckCheck}
                      title="Standard: The Eco-Plus Tier"
                      description="Weekly shredding for nutrients, plus one deep final property cleanup before winter hits."
                    />
                    <OptionCard
                      ...
                      description="The gold standard. Property stays 100% leaf-free every single week (Lawn, beds, and walkways)."
                    />
                  </div>
                </motion.div>
              )}
